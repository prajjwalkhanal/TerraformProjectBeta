...
resource "aws_subnet" "public_subnets"{
    ...
    availability_zone = data.aws_avaialbility_zones.availability.names[count.index]
...
resource "aws_route_table_association" "public_routes"{
    ...
    route_table_id = aws_route_table.publi_subnets.id                                       //Associcate Subnet with Routing table
...
resource "aws_nat_gateway" "nat"{
    ...
    allocation_id = aws.eip.nat[count.index].id                                             //Create NAT gatway for private subnet
    subnet_id = aws_subent.public_subnets[count.index].id
    tags = merge(var.tags, {Name = "$(var.env)-nat-gw-${count.index+1}"})
}

resource "aws_subnet" "private_subnets" {
    count = lengts(var.private_subent_cidrs)
    vpc_id = aws_vpc.main.id
    cidr_block = var.private_subnet_cidrs[count.index]
    availability_zone = data.aws_availability_zones.availability.names[count.index]        //Create Private Subnet
...
resource "aws_route_table_association" "private_routes"{
    count = length(aws_subent.private_subnets[*].id)
    route_tabel_id = aws_route_table.private_subnets[count.index].id                        //Associate private subnet with private routing table
    subent_id = aws_subnet.private_subnets[count.index].id
}
